...
## Instincts
- Situation: Editing public portfolio copy for GT-Launcher or related projects. Signal: any visitor-facing text, searchable tag, or public wiki reference contains LCARS, Star Trek, Starfleet, or old internal branding. Response: replace it with neutral product language while preserving implementation identifiers and visual tokens. Expected payoff: the public brand stays independent without unnecessary technical migration risk.
- Situation: A mobile app is distributed through its official store. Signal: a legacy APK download field shares the same URL but still renders as an APK download. Response: remove the download field and retain an explicitly labeled official-store link. Expected payoff: visitors reach the canonical install channel without stale file-version claims.
...
- [2026-09-16] Claude Kaira: Reverted the 2026-09-10/11 alazlabs.com rebrand. The `alazlabs.com` domain was never registered/pointed at Vercel (DNS never resolves); the live production alias has always been `alazlab.com`. Göktuğ confirmed `alazlab.com` is the correct domain. Fixed all `alazlabs.com` references back to `alazlab.com` across `src/` (layout metadata, sitemap, robots, OG image, all project/GTab/SVP pages), `package.json` name, `README.md`, and `gitrepo.md`. Also added new `/{lang}/svp`, `/{lang}/svp/privacy-policy`, and `/{lang}/svp/terms-of-service` pages so the SVP project's TikTok Developer app review has real Website/Privacy Policy/Terms of Service URLs.
- [2026-09-24] Codex Kaira: Removed GT-Launcher's legacy APK-download metadata while retaining the official Google Play link, relabeled the page and rail actions as Google Play navigation, and collapsed release notes behind a default-closed disclosure. Production build passes.
- [2026-09-24] Codex Kaira: Removed LCARS, Star Trek, Starfleet, and legacy internal-identifier references from all visitor-facing GT-Launcher content, the searchable project catalog, and the public GT-Launcher architecture guide. Styling tokens and application behavior remain unchanged; production build passes.
