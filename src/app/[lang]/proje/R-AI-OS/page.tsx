...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const project = getProjectBySlug('R-AI-OS', lang);
  if (!project) return {};
  ...
  return {
    ...
    openGraph: {
      title: `${title} — Göktuğ Turhan`,
      description,
      url: `https://alazlabs.com/${lang}/proje/R-AI-OS`,
    ...
    alternates: { canonical: `https://alazlabs.com/${lang}/proje/R-AI-OS` },
  };
}

const DEFAULT_STACK = ['Rust', 'Tokio', 'Ratatui', 'Axum', 'SQLite', 'MCP', 'fastembed'];

export default async function RAIOSPage({ params }: Props) {
  const { lang } = await params;
  const isEn = lang === 'en';
  const project = getProjectBySlug('R-AI-OS', lang);
  if (!project) notFound();
  ...
  // Load wiki docs
  const rawWikiDocs = getProjectWikiDocs('R-AI-OS', lang);
  ...
  const related = allProjects
    .filter(p => p.slug !== 'R-AI-OS' && (p.category === metadata.category || p.area === 'muhendislik'))
    .slice(0, 3);
  ...
  return (
    <article className="max-w-7xl mx-auto pb-24 px-2 sm:px-4 space-y-12 animate-in fade-in duration-500">
      ...
      {/* Flagship Interactive Rust CLI Terminal */}
      <RaiosClient techStack={metadata.techStack ?? DEFAULT_STACK} />
